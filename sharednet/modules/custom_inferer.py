...
__all__ = ["sliding_window_inference_cond", ]


def sliding_window_inference_cond(
    inputs_img_cond: Tuple[torch.Tensor, torch.Tensor],
    ...
    **kwargs: Any,
    ...
    _initialized = False
    for slice_g in tqdm.tqdm(range(0, total_slices, sw_batch_size), position=0, leave=True):
        slice_range = range(slice_g, min(slice_g + sw_batch_size, total_slices))
        unravel_slice = [
            [slice(int(idx / num_win), int(idx / num_win) + 1), slice(None)] + list(slices[idx % num_win])
            for idx in slice_range
        ]
        unravel_cond = [cond[int(idx / num_win)] for idx in slice_range]
        window_img = torch.cat([inputs[win_slice] for win_slice in unravel_slice]).to(sw_device)
        window_cond = torch.tensor(unravel_cond).to(sw_device)
        # print(f"window_cond: {window_cond}")
        # window_data = (window_img, window_cond)
        seg_prob = predictor(window_img, window_cond, *args, **kwargs).to(device)  # batched patch segmentation
...
class SlidingWindowInfererCond(Inferer):
    """
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        """
        return sliding_window_inference_cond(
            inputs,
            self.roi_size,
            self.sw_batch_size,
            network,
